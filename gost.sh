...
function Install_ct() {
  ...
  if test -a /usr/bin/gost -a /usr/lib/systemd/system/gost.service -a /etc/gost/config.json; then
    ...
    if [ -f "$SCRIPT_DIR/panel/install_panel.sh" ]; then
      echo -e "${Info} 正在安装 Web 管理面板..."
      bash "$SCRIPT_DIR/panel/install_panel.sh"
    fi
  else
    echo "gost没有安装成功"
    rm -rf "$(pwd)"/gost
...
————————————
 ...
 ${Green_font_prefix}12.${Font_color_suffix} 安装/管理 Web 可视面板
————————————" && echo
...
12)
  ...
  if [ -f "$SCRIPT_DIR/panel/install_panel.sh" ]; then
    ...
    echo -e "${Error} 未找到面板安装脚本，请确保 panel 目录完整"
  fi
  ;;
*)
  echo "请输入正确数字 [1-12]"
